...
async def _elegir(page: Page, select_sel: str, texto: str, *, timeout_ms: int = 10_000) -> bool:
    ...
    try:
        ...
        await page.keyboard.press("Escape")
        return False
    # El siguiente combo se habilita por una llamada al servidor: hay que
    # darle su tiempo, pero sin dormir a ciegas de más.
...
async def barrer(
    ...
    *,
    ...
    if not await _elegir(page, S.SELECT_SERVICIO, servicio):
        resultado.motivo_corte = f"no se pudo elegir el servicio '{servicio}'"
        return resultado
    if not await _elegir(page, S.SELECT_ENTIDAD, entidad):
        resultado.motivo_corte = f"no se pudo elegir la entidad '{entidad}'"
        return resultado
    ...
    for modulo in objetivo:
        ...
        if not await _elegir(page, S.SELECT_MODULO, modulo):
            resultado.hallazgos.append(
                Hallazgo(entidad, modulo, servicio, None, nota="no se pudo seleccionar el módulo")
            )
            continue
        ...
        # Sólo se afirma que hay disponibilidad si de verdad se vio un horario
        # ofrecido. Un calendario con días "libres" pero sin horarios no basta.
        ...
        resultado.hallazgos.append(
            Hallazgo(
                ...
                nota="" if hay is not None else "calendario sin señales claras (por confirmar)",
            )
        )
